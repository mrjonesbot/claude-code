...
This document outlines the core design principles and patterns used throughout the application. These principles should guide all UI/UX decisions and implementations, following S-tier SaaS dashboard standards inspired by industry leaders like Stripe, Airbnb, and Linear.

## Core Design Philosophy & Strategy

The application follows a **professional, clean, and functional** design approach. The interface embodies these fundamental principles:
...
> **Source of truth**: See `/context/style-guide.md` for the full brand color system (CSS custom properties defined in `application.css` using OKLCH values). Always use design system tokens (e.g., `bg-primary`, `text-primary`, `bg-danger`) — do NOT use raw Tailwind colors like `indigo-600` for brand elements.

#### Semantic Colors
- **Success**: Green-400 (icons), Green-100/800 (badges)
- **Danger**: `bg-danger` / `btn-danger` for buttons, Red-100/800 for badges
...
> **Source of truth**: See `/context/style-guide.md` for the full set of button variants and their CSS classes.
...
#### Link Button
```html
<button class="btn-link">Link Action</button>
```
